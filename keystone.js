...
function display_token_data(){
	...
	token_data.catalog.forEach(function (service, index, array){
		...
		if ((service.name == "keystone") && (endpoint.interface == "admin")){
...
function submit_token_request(){
    ...
    auth_method = $('input[name=auth_method]').val()
    ...
    if (scoped){
	token_request.auth['scope'] = jQuery.extend(true, {}, project_scoped_section)
...
function list_roles(){
    request_url = keystone_endpoint + "/roles"
	  ...
	  data.roles.forEach(function (role, index, array){
	       $("ol#global_role_list").append($("<li>").append(role.id + ":" +role.name ))
	  })
      },
